...
do
	...
	if [[ $ARCH == "linux-arm64"  ]]; then
		echo "linux-arm64"
		CGO_ENABLED=1 GOOS=$RELEASE_PLATFORM GOARCH=$RELEASE_ARCH go build -ldflags="-s '-extldflags=-static -lsharpyuv' -X 'github.com/tronbyt/pixlet/cmd.Version=${PIXLET_VERSION}'" -o "build/${RELEASE_PLATFORM}_${RELEASE_ARCH}/${PIXLET}" github.com/tronbyt/pixlet
		...
		go build -ldflags="-s '-extldflags=-static -lsharpyuv' -X 'github.com/tronbyt/pixlet/cmd.Version=${PIXLET_VERSION}'" -tags timetzdata -o build/${RELEASE_PLATFORM}_${RELEASE_ARCH}/${PIXLET} github.com/tronbyt/pixlet
		...
		echo "other"
		CGO_CFLAGS="-I/tmp/${LIBWEBP_VERSION}/${ARCH}/include" CGO_LDFLAGS="-L/tmp/${LIBWEBP_VERSION}/${ARCH}/lib" CGO_ENABLED=1 GOOS=$RELEASE_PLATFORM GOARCH=$RELEASE_ARCH go build -ldflags="-X 'github.com/tronbyt/pixlet/cmd.Version=${PIXLET_VERSION}'" -o "build/${RELEASE_PLATFORM}_${RELEASE_ARCH}/${PIXLET}" github.com/tronbyt/pixlet

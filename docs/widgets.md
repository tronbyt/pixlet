...
def main():
    # Use a hex string
    red_box = render.Box(color="#f00")

    # Use a Color object
    blue = color.rgb(0, 0, 255)
    # could also be defined as color.hex("#00f") or color.hsv(0, 0, 1)
    blue_box = render.Box(color=blue)
    # ...
```
...
| `pad` | `int / tuple of 3 ints` | Padding around the child | N |
